    ...
    axios.get(`${this.store.baseUrl}/api/doctors`).then((response) => {
      ...
      if (response.data && response.data.results) {
        ...
        this.doctors.forEach((doctor) => {
          ...
          axios.get(`${this.store.baseUrl}/api/doctor/${doctor.id}/average-vote`).then((response) => {
            // Verifica che la risposta contenga i dati della media dei voti
            if (response.data && response.data.average_vote) {
              // Assegna la media dei voti al dottore corrispondente
              doctor.averageRating = response.data.average_vote;
            } else {
              console.error('La risposta API non contiene la media dei voti:', response.data);
            }
          })
          .catch((error) => {
